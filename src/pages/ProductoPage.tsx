...
const productToCut: Record<string, string> = {
...
export default function ProductoPage() {
  ...
  const cutId = productToCut[producto.nombre]
  const related = productos.filter(p => p.id !== producto.id).slice(0, 3)
  const waUrl = `https://wa.me/5492616401013?text=${encodeURIComponent(`Hola! Quiero consultar sobre ${producto.nombre}`)}`
    ...
    <div className="min-h-screen bg-dark pt-20">
      ...
      <div className="mx-auto max-w-7xl px-5 py-4 sm:px-6 lg:px-8">
        <div className="flex items-center gap-2 text-xs text-cream/30">
          ...
          <span>/</span>
          <Link to="/#productos" className="hover:text-cream transition-colors">Productos</Link>
          <span>/</span>
        ...
        <div className="grid gap-10 lg:grid-cols-2 lg:gap-16">
          ...
          <div>
            <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }}
              ...
              {producto.badge && (
                <span className="absolute top-4 left-4 bg-campo/90 px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-cream">
                  {producto.badge}
                </span>
              )}
            </motion.div>
            ...
            {cutId && (
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
                ...
                <div className="mb-3 flex items-center gap-3">
                  <div className="h-px w-8 bg-campo" />
                  <span className="text-[10px] font-medium uppercase tracking-[0.2em] text-campo">
                    Ubicación del corte
                  </span>
                </div>
                <CowDiagram activeCut={cutId} interactive={false} />
                {/* Anatomical caption — desktop only */}
                <p className="mt-3 hidden text-[10px] leading-relaxed text-cream/35 italic sm:block">
                  {producto.ubicacion}
                </p>
              </motion.div>
            )}
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="mb-3 flex items-center gap-3">
              ...
              <span className="text-[10px] font-medium uppercase tracking-[0.2em] text-campo">{producto.corte}</span>
            </div>
            ...
            <div className="mt-2 flex flex-wrap gap-1.5">
              {producto.nombresAlternativos.map(n => (
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {producto.metodos.map(m => (
                <span key={m} className="border border-campo/25 bg-campo/8 px-3 py-1 text-[9px] font-semibold uppercase tracking-wider text-campo/80">
                  {m}
                </span>
              ))}
            </div>

            {/* Info sections — stacked, full width, readable */}
            <div className="mt-6 divide-y divide-dark-border border border-dark-border">
              <div className="p-4 sm:p-5">
                <p className="mb-2 flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.15em] text-campo">
              ...
              <div className="p-4 sm:p-5">
                ...
                <p className="text-sm leading-relaxed text-cream/55">{producto.idealPara}</p>
              </div>
            </div>

            {/* Pro tip */}
            <div className="mt-4 border-l-2 border-campo bg-campo/5 px-4 py-3 sm:px-5">
              <p className="text-[10px] font-bold uppercase tracking-[0.15em] text-campo">Pro tip del asador</p>
              <p className="mt-1.5 text-sm leading-relaxed text-cream/60">{producto.proTip}</p>
            </div>

            {/* CTA */}
            <div className="mt-6 flex flex-col gap-3 sm:flex-row">
              <a href={waUrl} target="_blank" rel="noopener noreferrer"
                className="flex flex-1 items-center justify-center gap-2 bg-campo px-6 py-3.5 text-xs font-semibold uppercase tracking-wider text-cream transition-all hover:bg-campo-light">
              ...
              <Link to="/#productos"
                className="flex items-center justify-center border border-dark-border px-6 py-3.5 text-xs font-semibold uppercase tracking-wider text-cream/50 transition-all hover:border-cream/30 hover:text-cream">
          ...
          <div className="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-3">
            {related.map(p => (
              <Link key={p.id} to={`/producto/${p.id}`}
                className="group flex gap-4 border border-dark-border bg-dark-card p-3 transition-colors hover:border-campo/30">
                <img src={p.imagen} alt={p.nombre} className="h-20 w-20 object-cover" />
                <div className="flex flex-col justify-center">
                  <h4 className="font-heading text-sm font-bold text-cream group-hover:text-campo transition-colors">{p.nombre}</h4>
